...
write_files:
 ...
 - content: |
    module.exports = {uiPort: process.env.PORT || 443, requireHttps: true, https: {key: require("fs").readFileSync('/root/.node-red/node-key.pem'),cert: require("fs").readFileSync('/root/.node-red/node-cert.pem')}, mqttReconnectTime: 15000, serialReconnectTime: 15000, debugMaxLength: 1000, httpAdminRoot: '/nadmin', adminAuth: {type: "credentials", users: [{username: "Ameren", password: "$2b$08$Rx8EGoP8uZmLFzA.9S1CMebrt159MLtxRcCwfi8r27N2BbBDOPb1K", permissions: "*"}] }, logging: {console: {level: "info", } } }
