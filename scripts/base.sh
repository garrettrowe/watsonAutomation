...
export USER=root
apt-get update
curl -d "Instance=$(< instnum.txt)&Log=Patching VSI" -X https://daidemos.com/log
apt-get -y -o Dpkg::Options::="--force-confnew" upgrade
curl -d "i=$(< instnum.txt)&Log=Installing Core Packages" -X POST https://daidemos.com/log
apt-get -y -o Dpkg::Options::="--force-confnew" install libcurl4 libssl1.1 build-essential fdupes
curl -d "Instance=$(< instnum.txt)&Log=Installing Node" -X POST https://daidemos.com/log
...
wget -O /root/.node-red/flows_$(< companysafe.txt)-vsi.json https://raw.githubusercontent.com/garrettrowe/watsonAutomation/main/demos/watson/flows.json
curl -d "Instance=$(< instnum.txt)&Log=Starting Data Aggregator" -X POST https://daidemos.com/log
...
wget -O /root/companylogo.png https://daidemos.com/$(< company.txt).png
wget -O /root/companyurl.txt https://daidemos.com/$(< company.txt).txt
...
curl -d "Instance=$(< instnum.txt)&Log=Starting Services" -X POST https://daidemos.com/log
systemctl enable nodered.service
systemctl start nodered.service
curl -d "Instance=$(< instnum.txt)&Log=Provision complete! Content loading and model training will occur in the background over the next few hours." -X POST https://daidemos.com/log
curl -d "Instance=$(< instnum.txt)" -X POST https://daidemos.com/complete
